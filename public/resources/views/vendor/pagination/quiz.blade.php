@if ($paginator->hasPages())
            ...
            @else
                <a href="{{ route('home') }}" class="btn btn-primary btn-block btn-lg" rel="next" aria-label="@lang('pagination.next')">{{\App\EventDetail::whereName('finish_survey_button')->first()->content}}</a>
            @endif
@else
            <a href="{{ route('home') }}" class="btn btn-primary btn-block btn-lg" rel="next" aria-label="@lang('pagination.next')">{{\App\EventDetail::whereName('finish_survey_button')->first()->content}}</a>
@endif